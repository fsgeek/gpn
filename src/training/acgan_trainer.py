"""
AC-GAN Trainer for single-digit MNIST.
...
from src.models.acgan import ACGANGenerator, ACGANDiscriminator
...
class ACGANTrainer:
    ...
    def __init__(
        ...
        logger.info("ACGANTrainer initialized")
    ...
    def train(
        ...
        self.logger = MetricsLogger(log_dir, experiment_name="acgan")

        logger.info(f"Starting AC-GAN single-digit training for {total_steps} steps")
    ...
    def _save_checkpoint(self, suffix: Optional[str] = None) -> None:
        ...
        if suffix:
            path = checkpoint_dir / f"acgan_{suffix}.pt"
        else:
            path = checkpoint_dir / f"acgan_step{self.current_step}.pt"
