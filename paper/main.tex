...
\begin{table}[t]
    \centering
    \caption{Compositional transfer accuracy on held-out relational pairs (MNIST: 5 seeds, Fashion-MNIST: single run). Gap is statistically significant: $p = 0.0056$, Cohen's $d = 3.31$.}
    \label{tab:composition}
    \begin{tabular}{lcc}
        \toprule
        Training Paradigm & MNIST & Fashion-MNIST \\
        \midrule
        Adversarial (GAN) & $80.1 \pm 2.8\%$ & 79.5\% \\
        Pedagogical (GPN) & $96.7 \pm 6.5\%$ & 100\% \\
        \midrule
        Gap & +16.6\% & +20.5\% \\
        \bottomrule
    \end{tabular}
...
The 81\% adversarial ceiling is Personality Engineering Failure. Adversarial training finds outputs that satisfy the discriminator but doesn't bound the modification class toward compositional structure. The generator learns surface patterns that fool the discriminator but don't generalize to novel compositions.
